...
export const AccountTransferPage = () => {
  ...
  return (
    <Box display="flex" flexDir="column" gap="s16">
      ...
      {paymentStatus === 'review' && <AccountTransferReview setPaymentStatus={setPaymentStatus} />}

      {paymentStatus === 'success' || paymentStatus === 'failure' ? (
        <AccountTransferResult paymentStatus={paymentStatus} setPaymentStatus={setPaymentStatus} />
      ) : null}
    </Box>
  );
};
