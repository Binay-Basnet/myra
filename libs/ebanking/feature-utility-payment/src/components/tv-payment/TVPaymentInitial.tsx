...
export const TVPaymentInitial = ({ setCurrentSequence }: InternetPaymentProps) => {
  ...
  const { data: internetUtilitiesData } = useListUtilitiesQuery({
    ...
    filter: {
      orConditions: [
        { andConditions: [{ column: 'servicename', comparator: 'EqualTo', value: 'tv' }] },
      ],
    },
  });
    ...
    >
      <Box p="s16" display="flex" flexDir="column" gap="s32">
        ...
        <FormSelect
          ...
          rules={{ required: 'TV Provider is required' }}
        />
