...
export const PackagePaymentInitital = ({ setCurrentSequence }: InternetPaymentProps) => {
  ...
  const { data: packageUtilitiesData } = useListUtilitiesQuery({
    pagination: { ...getPaginationQuery(), first: -1 },
    filter: {
      orConditions: [
        { andConditions: [{ column: 'servicename', comparator: 'EqualTo', value: 'package' }] },
      ],
    },
  });
  ...
  const packageOptions = useMemo(
    () =>
      packageUtilitiesData?.eBanking?.utility?.listUtilities?.edges?.map((internet) => ({
        label: internet?.node?.name as string,
        value: internet?.node?.slug as string,
      })) ?? [],
    [packageUtilitiesData]
  );
