...
import { PackagePaymentInitital } from '../components/package';

type PaymentStatus = 'form' | 'review' | 'success' | 'failure' | 'pending';

export const UtilityPackagePayment = () => {
  ...
  return (
    <FormProvider {...methods}>
      <Box display="flex" flexDir="column" gap="s16">
        <EbankingPathBar
          paths={[{ label: 'Utility Payments', link: '/utility-payments' }, { label: 'Package' }]}
        />

        <Box display={paymentStatus === 'form' && currentSequence === 0 ? 'block' : 'none'}>
          <PackagePaymentInitital setCurrentSequence={setCurrentSequence} />
        </Box>
