...
export const AdjustedTransactionsList = () => {
  ...
  const columns = useMemo<Column<typeof rowData[0]>[]>(
    () => [
      ...
      {
        header: 'Fiscal Year',
        accessorFn: (row) => row?.node?.yearEndAdjustment,
      },
      {
  ...
  return (
    <>
      <TransactionPageHeader heading="Adjusted Transactions" />
      ...
      <Table
        ...
        columns={columns}
        rowOnClick={(row) =>
          router.push(`${ROUTES.CBS_TRANS_ADJUSTED_TRANSACTIONS_DETAILS}?id=${row?.node?.id}`)
        }
        pagination={{
          total: data?.transaction?.listAllTransactions?.totalCount ?? 'Many',
          pageInfo: data?.transaction?.listAllTransactions?.pageInfo,
        }}
        searchPlaceholder="Search adjusted transactions"
        menu="TRANSACTIONS"
      />
    </>
  );
