...
import { useGetOpenChargeQuery } from '@coop/cbs/data-access';
import { localizedDate } from '@coop/cbs/utils';

interface IUpdateOpenChargeDetailModalProps {
  isOpen: boolean;
  onClose: () => void;
  serviceId: string;
}

export const UpdateOpenChargeDetailModal = ({
  isOpen,
  onClose,
  serviceId,
}: IUpdateOpenChargeDetailModalProps) => {
  const { data } = useGetOpenChargeQuery({ id: serviceId }, { enabled: !!serviceId });
  const openServiceChargeData = data?.settings?.general?.depositProduct?.getOpenCharge?.data;
  return (
    <Modal title="Account Open Charge Update" open={isOpen} onClose={onClose}>
