...
export const DailyInterestBooking = () => {
  ...
  const { data: eodDetailsData, isFetching } = useEodHistoryDetailsQuery({
    ...
    filter: {
      ...
      jobType: 'SAVING_INTEREST_BOOKING',
  ...
  const columns = useMemo<Column<typeof detailList[0]>[]>(
    () => [
      ...
      },
      {
        ...
        accessorFn: (row) => amountConverter((row?.payload?.closing_balance ?? 0) as number),
        meta: {
          isNumeric: true,
        },
      },
