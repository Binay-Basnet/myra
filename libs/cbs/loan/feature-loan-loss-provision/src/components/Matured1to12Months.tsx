...
export const Matured1to12Months = ({
  setSelectedAccounts,
}: {
  ...
  const columns = useMemo<Column<typeof oneTo12Months[0]>[]>(
    () => [
      ...
      },
      {
        ...
        accessorKey: 'amount',
        cell: (props) => amountConverter((Number(props?.row?.original?.amount) * 0.35).toFixed(2)),
        meta: {
          isNumeric: true,
        },
      },
  ...
  return (
    <>
      <FormSection templateColumns={1} header="Matured (Due) - 1 to 12 months" divider={false}>
        <Table
          data={oneTo12Months}
          columns={columns}
          isStatic
      ...
      <FormSection header="Ledger Mapping for Matured (Due) - 1 to 12 months">
        <Box display="flex" flexDirection="column" gap="s4">
          <Text variant="formLabel">From</Text>
          <FormCOASelectModal name="provision1To12M.provisionMapping.from" />
        </Box>
        <Box display="flex" flexDirection="column" gap="s4">
          <Text variant="formLabel">To</Text>
          <FormCOASelectModal name="provision1To12M.provisionMapping.to" />
        </Box>
      </FormSection>
    </>
  );
