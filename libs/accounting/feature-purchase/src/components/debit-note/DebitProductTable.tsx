...
export const DebitProductTable = () => {
  ...
  const tableColumns: Column<SalesTable>[] = [
    ...
    },
    {
      ...
      accessorFn: (row: SalesTable) => {
        const value = String(Number(row.quantity || 0) * Number(row.rate || 0));
  ...
  return (
    <FormSection>
      <GridItem colSpan={3}>
        <FormEditableTable<SalesTable> name="itemDetails" columns={tableColumns} />
      </GridItem>
    </FormSection>
  );
};
