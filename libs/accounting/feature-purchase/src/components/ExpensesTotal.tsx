...
export const ExpensesTotal = () => {
  ...
  const { watch } = useFormContext<AccountingExpenseInput>();

  const [grandTotal, setGrandTotal] = useState<number>(0);

  const itemDetails = watch('transferredLedgers') as ExpenseLedgerTransfer[];
  ...
  const totalVat = itemDetails?.reduce(
    (a, b) =>
      a +
      (Number(b?.amount) *
        Number(
          taxData?.settings?.general?.accounting?.taxRates?.find((tax) => tax?.id === b?.tax)
            ?.rate || 0
        )) /
        100,
    0
  ...
  return (
    <Box display="grid" gap="s32" gridTemplateColumns="repeat(2,1fr)" p="s20">
      <FormTextArea name="note" label={t['invFormNotes']} rows={5} />
